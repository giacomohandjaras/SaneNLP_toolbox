function [r,rho,acc,RDM_behav,RDM_corpus,concepts_labels,coverage]=SNLP_benchmark_fra_wordnorms(corpus,smoothing)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%Test word-embeddings from an french-based corpus with behavioral feature-norming data from Buchanan et al. semantic word-pair norms (2013).
%%%%Words from Buchanan et al. (2013) were translated by giada lettieri.
...
%%%%
%%%%Usage:
%%%%	[r,rho,acc,RDM_behav,RDM_corpus,concepts_labels,coverage]=SNLP_benchmark_fra_wordnorms(corpus,smoothing)
%%%%
...
load 'SNLP_benchmark_fra_wordnorms.mat';
...
[vectors_mask] = SNLP_searchWords(words_fra, corpus.wordlist);

all_words(find(vectors_mask==0))=[];
words_fra_stem(find(vectors_mask==0))=[];
words_fra(find(vectors_mask==0))=[];
num_of_stim=numel(words_fra);

concepts_labels=words_fra;
...
for j=1:iterations
    ...
    if(smooth>0)
        wordvectors_extracted = SNLP_getWordVectorsSmoothed(words_fra, corpus.wordlist, corpus.vectors_matrix, words_fra_stem, corpus.wordlist_stem, smooth);
    else
        wordvectors_extracted = SNLP_getWordVectors(words_fra, corpus.wordlist, corpus.vectors_matrix);
    end
